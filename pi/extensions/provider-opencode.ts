...
export default async function (pi: ExtensionAPI) {
  ...
  // opengo-cloud-lite  → lightest model, essentially unlimited quota
  // opengo-cloud-auto  → best model with cascading fallbacks on exhaustion
  // opengo-cloud-pro   → smartest/best model available

  const LITE_MODEL = "minimax-m2.5-free";
  const STANDARD_MODEL = "deepseek-v4-flash-free";
  const PRO_MODEL = "deepseek-v4-flash";
  ...
  const AUTO_CHAIN: Tier[] = [
    ...
    { baseUrl: FREE_BASE, model: LITE_MODEL },
  ];
  ...
  function resolveTiers(requestedModel: string): Tier[] {
    switch (requestedModel) {
      ...
      case "opengo-cloud-auto":
        return AUTO_CHAIN;
      default:
        return [{ baseUrl: FREE_BASE, model: requestedModel }];
    }
  }
